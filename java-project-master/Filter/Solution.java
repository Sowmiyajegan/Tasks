...
public class Solution {
	...
	public static void main(String[] args) throws IOException, SQLException {
		...
		item[0].seteDate("09-9-2016");
		item[0].setmDate("29-1-2012");
		session.save(item[0]);
		item[1]=new Item();
		item[1].setiName("PENCIL");
		item[1].setPrice(10);
		item[1].setQuantity(10);
		item[1].seteDate("09-9-2016");
		item[1].setmDate("29-1-2012");
		session.save(item[1]);		
		User[] user=new User[3];
		user[0]=new User();
		user[0].setUserName("naveen");
		user[0].setPassword("nav");
		session.save(user[0]);
		user[1]=new User();
		user[1].setUserName("nivetha");
		user[1].setPassword("niv");
